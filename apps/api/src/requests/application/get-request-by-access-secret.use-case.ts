...
import { isLiveRequestPublicNumber, LIVE_LETTER_READING } from '../domain/live-request-fixture.js';
...
export class GetRequestByAccessSecretUseCase {
  ...
  async execute(accessSecret: string): Promise<RequestPortalView> {
    ...
    return {
      ...
      files: record.files,
      ...(isLiveRequestPublicNumber(record.publicNumber)
        ? { demoLive: true as const, letterReading: LIVE_LETTER_READING }
        : {}),
    };
  }
}
