...
`nestjs-core`, `openapi-client-core`). Предметный продукт (фичи 1–27 и live 28–33)
реализуй **только** по промпту `docs/llm/feature-NN.md` в отдельном чате, один PR
на фичу. Доработки UX/UI понятности — **только** по `docs/ux/task-NN.md` (вход:
...
(`docs/remediation-plan.md`, D-030). Фраза `выполни фичу N` — live-сценарий
показа (D-049, фичи 28–33). Фраза `реализуй вынос ядра` — генератор второго
workspace (`docs/llm/scaffold-new-workspace.md`, D-056), не фича 1–33 и не UX.
Не копируй
...
Демо канала статуса по не-SKU заявке ПК «Нордщит»: служебный список ссылок, кабинет
снаружи по секрету в URL (принят → в расчёте → КП → счёт), вход «подать заявку» и
пульт ведущего для одной живой заявки. Зритель за 30 секунд должен понять: заказчик
видит шаг по ссылке, без кабинета с логином. Вымысел, не кейс и не витрина Вольтариса.
...
пункт» и фразы `выполни задачу N` / `выполни ux задачу N` / `выполни фичу N`
без просьбы цикла — по-прежнему один чат по промпту фичи/UX. `реализуй вынос ядра` —
один чат по `docs/llm/scaffold-new-workspace.md`, не цикл.
