...
Пользователь написал «выполни ux задачу 3». Этот файл — полный вход. Другой
...
1. **До Vue** заменить/добавить asserts в e2e (старые сценарии не `skip`):
   ...
   - **Заменить**, не `skip`, существующие
     `getByRole('heading', { name: quoteCabinet.publicNumber })` и
     `getByRole('heading', { name: 'З-10042' })` в том же
     `e2e/request-cabinet.spec.ts` (сейчас около строк 31, 64, 175, 203, 217):
     номер виден как текст / `tabular-nums`, heading с номером = 0. Тест З-10042:
     h1 = `В расчёте`. Возврат с листа и клик с индекса проверяют кабинет с h1
     `КП готово`, не heading номера.
...
Skill `pr-review`. Дальше: «выполни ux задачу 4» → `docs/ux/task-04.md`.
