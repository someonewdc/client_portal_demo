...
есть comment). Заголовок «Кол-во» — одна строка: `scrollHeight` ячейки `th`
совпадает с соседним `th` «Ед.» (±2px) и в тексте нет перевода строки
(`clientHeight` однострочный, либо `getClientRects().length === 1`).
...
1. Playwright кабинет З-10043, 1280×900, **до** правки:
   - `th` с текстом exact `Кол-во` имеет одну клиентскую строку
     (`element.getClientRects().length === 1`);
   - высота этого `th` ≤ высота `th` «Наименование» + 4px (не двухстрочный
     заголовок при однострочном «Наименование»);
   - ячейки `1` / `шт` / `IP54, навесной` видны;
   - 390×844: подписи блоков задачи 8 и `scrollWidth <= 390` живы.
2. `pnpm test:e2e` — **red** на 1280px (`Кол-во` сейчас в две строки).
