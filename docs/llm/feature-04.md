...
- `docs/domain-model.md`, `docs/decisions.md` (D-005, D-008, D-012)
- `docs/testing.md`
...
1. Напиши Playwright по AC: дисклеймер, подпись про мессенджер, 5 номеров З-10041…,
   клик по З-10043 ведёт на `/r/seed-z10043-quote-kuznetsov`.
2. Запусти `pnpm test:e2e` — **red** (нет списка / нет текста).
3. Потом страница.
...
- Given seed и живые api+web, When открыть `/`, Then виден дисклеймер и 5 заявок с
  контрагентами и штампами из API.
- When клик по З-10043, Then URL `/r/seed-z10043-quote-kuznetsov`.
- Нет ссылок `href="#"` и кнопок без действия.
- E2E написан до реализации страницы (есть red evidence в status).
